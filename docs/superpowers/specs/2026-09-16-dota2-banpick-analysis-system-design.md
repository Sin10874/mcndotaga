...
**实测口径（2026-09-18 澄清）**：约 0.6% 是在 **1,014 场（几乎全为 24 手）的 OpenDota 样本**上测得的（1,014 场中 6 场仅 23 手，个别 `ord` 类型反转）—— 该样本整个落在同一个手序族的时代里，所以它只说明"24 手模板内部很稳定"，**不能**当作全库口径。全库（2016–2026，211,051 场）按**澄清后的族规则**（十种合法族，见 §6.0 澄清）实测 **0.97%**（2,048 场）：手数未登记（10–19 手 1,060 场、21 手 262 场、23 手 640 场等）与"手数已登记但顺序对不上任何族"（20/22 手尾部 81 场）两类。**时代不同但合法的手序族（20/22 手的历史顺序）不算异常。**

| 情形 | 处理 |
|---|---|
| 入库 | **不阻断**。写入实际拿到的 `draft_actions`，设 `n_draft_actions`、`anomaly=true`，并在 `draft_anomalies` 记一行 |
| UI | 标注"该场 BP 结构异常"，相关统计默认**排除**，可显式包含 |
| 序列模型（§10.1） | **默认排除**出训练与评估；`anomaly=false` **且** `draft_state='complete'` 是入模的硬条件（完整口径与族对齐要求见 §10.1） |
| 模板推出的队伍归属 | 仍然按 `first_pick_team` + 模板推导——该规则在 1,014 场上零反例，与手数异常无关（历史族的族名与逐手映射见 §6.0 澄清） |
...
**`TEMPLATE` 是「2026 年族」的定义，不是全库唯一的合法顺序**（2026-09-18 澄清，实测见 §16.3 与计划 Task 12；此前把 1,014 场样本上的结论当成了全库结论）。**Valve 改过 CM 的 ban 顺序**：2016–2026 的语料（211,051 场）里实际存在 **20 / 22 / 24 三种手数与十种支持度 ≥ 1% 的合法手序族**（逐族定义见 `ingest/order_families.py` 的 `DRAFT_ORDERS`）。因此：

- `resolve()` **只对 `spec_6_0_24` 族正确** —— 上面的 `TEMPLATE` 就是那一族，也是**当下**（2025 下半年起 + 全部 2026 目录；在有 BP 序列的 2026 场次里实测 14,131/14,189 = 99.59%）的 CM 顺序；
- **本契约不变**：契约示例与前端一律以这个 24 手模板为准，本节不为历史族新增参数或返回值；**族感知下沉到 ingest/analysis/models**；
- **入库层必须判定每场的族**：族由 `n_draft_actions` + `first_pick_team` + 该场的 `draft_actions` **唯一复算**（`ingest.order_families.family_for()`），是**派生量、不落库**（`matches` 没有族列）；下游对非 `spec_6_0_24` 的场次**不得**用本节的 `resolve()` 推 `(is_pick, team)`；
- **时代不同但合法的手序不是异常**：`anomaly=false` 且 `draft_state='complete'`。`anomaly=true` 的语义是「无法与**任何**已登记的合法 CM 顺序结构对齐」（手数不落在 {20, 22, 24} 也计入）；
- `< 2%` 的异常率判据是**全库口径**（见 §15），**不得**只在 24 手子集上断言 —— 该子集实测 0/149,522，任何阈值都恒真，断言会空过。
...
**2026 年现行 CM 顺序**已完整验证（OpenDota 1,014 场样本，零例外；**历史手序族见 §6.0 澄清** —— 那 1,014 场几乎全为 24 手，代表的是一个时代而不是全库）。令 `F` = 先手方（`first_pick_team`），`O` = 后手方：
...
- **异常序列 / 可入模条件**：入模的硬条件是 **`anomaly = false` 且 `draft_state = 'complete'`**（两个都要满足），不是偏好。全库异常率实测 **0.97%**（§5.3）；而实测还有 **6,046 场 `anomaly=false` 但 `draft_state='pending'`**（有 metadata、`picks_bans` 尚未到，根本没有 BP 序列）—— 只筛 `anomaly=false` 会把它们当成"0 手序列"喂进模型。**并且必须按 `order_family` 对齐**：§6.0 的 `resolve()` 只对 `spec_6_0_24` 族正确，历史族的位置 token 必须由 `ingest.order_families.resolve_in()` 推出（见 §6.0 澄清）。
...
| **数据完整性测试** | **不得**断言"每场必须 24 行、14 ban + 10 pick"——Valve 改过 CM 手序，2016–2026 语料里实测有**十种支持度 ≥ 1% 的合法族**（20/22/24 手，见 §6.0 澄清），全库异常率 **0.97%**（2,048/211,051）。改为：① 按**全库**分母断言偏离率低于 2% 的**统计阈值**（**不得**把分母限制到 24 手子集：该子集实测 0/149,522，任何阈值都恒真、断言会空过）；② 逐场偏离记录到 `draft_anomalies` 并标记 `draft_state='complete'` 但附加 `anomaly=true`，且**时代不同但合法的手序族不算异常**；③ 每个 `anomaly=false` 且有 BP 序列的场次必须命中**恰好一个**已登记族（**零反例**，实测 202,957/202,957），并报告逐族分布（24 手五族 68,775 / 46,347 / 15,135 / 14,045 / 5,220；22 手三族 26,126 / 8,035 / 4,767；20 手两族 12,340 / 2,167）；④ 队伍归属与 `first_pick_team` 一致（此条**可硬断言**，实测 211,051/211,051 成立）；⑤ 契约示例只校验 `spec_6_0_24` 族——其余族的 `ord→(type, team)` 由 `ingest.order_families.resolve_in()` 给出 |
...
| **模板推导测试** | 对每一场断言 `(ord, team, is_pick)` 与**该场所属族**的 `resolve` 一致（`anomaly=false` 的场次必须零反例）：`spec_6_0_24` 族用 §6.0 的 `resolve(ord, first_pick_team)`，其余九族用 `ingest.order_families.resolve_in(family, ord, first_pick_team)`（见 §6.0 澄清：契约示例只覆盖 `spec_6_0_24`，历史场次按它推导会整体错位）。契约示例中的 `draft` 必须与该场真实数据一致——**防止示例与规则再次漂移** |
