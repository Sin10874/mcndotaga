...
<!-- FILE: tests/test_m1_acceptance.py -->
```python
...
```bash
git add tests/test_m1_acceptance.py docs/superpowers/plans/2026-09-16-contract-freeze-and-data-foundation.md
git commit -m "test(m1): M1 验收门（常量/版本/引导数据集/契约/目录边界，数据缺失即失败不跳过）"
```

**M1 验收对照**：

| 规格 §14 M1 条款 | 落地断言 | 单条守护（各自文件） |
|---|---|---|
| 常量表英雄 = 127、道具 = 501 | Task 13 `M1-1`（逐值 + 快照行） | Task 11 `test_load_constants_populates_all_tables` |
| 版本表含 84 个字母子版本 | Task 13 `M1-2`（84 字母 + 118 总数） | Task 10 `test_valve_list_has_118_versions_and_84_lettered` |
| 引导数据集 506 MB 子集入库 | Task 13 `M1-3`（211,051 / 4,772,342 / 1,557） | Task 12 `test_bootstrap_loaded_a_substantial_number_of_matches` |
| `anomaly=true` 占比 < 2% | Task 13 `M1-4`（全库分母 + 绝对数 ±5% + kinds 合法性） | Task 12 `test_anomaly_rate_under_2_percent` |
| `matches`/`draft_actions`/`leagues` 可查 | Task 13 `M1-5`（`SELECT 1 ... LIMIT 1` + 精确计数） | Task 12 `test_draft_actions_and_leagues_are_queryable` |
| §14 M0 行（契约 / fixture / 目录边界） | Task 13 `M0-B1`–`B4`（**Task 13 是本计划最后一个能钉住它们的机会**） | Task 5–8 的 `tests/contracts/`（**不含**文档级 OpenAPI 校验与目录边界，那两条为本文件独有） |
| §15 的判据（异常口径 / 族全域分类 / 归属边界 / token 派生） | Task 13 `M1-6`–`M1-10`（与入库状态逐值对齐） | `tests/ingest/test_order_families.py`、`tests/ingest/test_bootstrap.py`、`tests/constants/test_load.py` |

**以上覆盖不到、且没有任何测试覆盖的 M1 相关项**（写在这里以免「全绿」被误读为「全验」）：

1. **`patch` 列交叉校验的逐场归因**：实测 191,149/193,499 吻合（98.79%），2,350 场不吻合，
   逐场原因未查（Task 12 实测记录的未验证项）。下限守护在 `tests/ingest/test_bootstrap.py`。
2. **`start_date_time` 的时区假设**：整条归属链建立在"朴素字符串按 UTC 解释"之上，只有
   `patch` 列的间接印证，没有独立时区证据。
3. **`draft_timings.csv` 未入库**、`teams` 表仍为 0 行（实测 150,809 处队名引用写 NULL）——
   两者都是 Task 12 的有意范围（规格 §10.2 的补充来源、Phase A 不含 `*/teams.csv`）。
4. **规格 §5.1 要求 3 的跨快照不变性**：需要两个快照并存，归 Plan 2（见文末「显式延迟」第 4 项）。

### Task 13 实测记录（2026-09-18，提交前实跑）

命令（DB env 与计划一致；`make` 目标在本机不可用，一律走 `.venv/bin/python -m ...`）：
...
.venv/bin/pytest tests/test_m1_acceptance.py -q          # 验收本体
.venv/bin/pytest -q                                      # 全量
```

**(a) 验收本体（缓存在位）——`1 passed`，耗时 2 分 41 秒（含一次 211,051 场的引导入库）**：

```
M1 实测：matches=211051 draft_actions=4772342 leagues=1557 patches=118/84 字母 heroes=127
        items=501 anomaly=2048/211051=0.97% pending=6046 pre2018=17552 已分类=202957/202957
1 passed in 161.80s (0:02:41)
```

loader 自己打印的旁证（同一次入库）：`folders=19 matches=211051 draft_actions=4772342
leagues=1557（逐目录累计 1808，跨目录重复） teams=0 anomalies=2048 pending=6046
丢弃的越界手=0 重复 ord=0`；`pre-2018 17552/211051（8.3%）` 且**全部** `patch_id IS NULL`，
`>= 1517472000` 的场次中 NULL 的 **0** 场；族分布
`{cm20_a 12340, cm20_b 2167, cm22_a 26126, cm22_b 8035, cm22_c 4767, cm24_a 68775,
cm24_b 46347, cm24_c 14045, cm24_d 5220, spec_6_0_24 15135}`（合计 202,957）。
所有数字与规格 §15/§16.3 及 Task 12 的实测表**逐项相同**。

**(b) 全量套件 `pytest -q`**：**`216 passed in 196.76s (0:03:16)`** —— 215 + 本文件 1 条，
**零 skip、零 failed**。（缓存在位时数据侧测试不再 skip，故那 13 个 skip **只**在缓存缺失时
出现；见 (c) 的 `202 passed, 13 skipped`。）

**(c) 空过证明：把 `tests/fixtures/kaggle` 移走 → `1 failed`（不是 `1 skipped`）**：

```
$ mv tests/fixtures/kaggle tests/fixtures/kaggle__moved_aside
$ pytest tests/test_m1_acceptance.py -q
E   Failed: M1 验收未通过（1 条）：
E   ✗ M1-A 引导数据集在位（§14 M1「506 MB 子集入库」）：实测 '无 tests/fixtures/kaggle/'，要求 缓存存在
E     —— 缺少 Kaggle 引导数据集缓存 …/tests/fixtures/kaggle。该数据集是 CC0 公共数据集，
E     **无需 Kaggle 凭证**：直接运行 `python -m ingest.kaggle_subset` 即可下载 506.7 MB 子集
E     （先看一眼要下什么：`python -m ingest.kaggle_subset --list-only`）；
E     或把已有的 CSV 放到 …/tests/fixtures/kaggle/<年度>/。
1 failed in 0.38s
$ pytest -q                      # 全量：套件其余部分行为不变，零失败
1 failed, 202 passed, 13 skipped in 1.73s
$ mv tests/fixtures/kaggle__moved_aside tests/fixtures/kaggle   # 复原（49 个 CSV / 483 MB，逐名核对）
$ pytest tests/test_m1_acceptance.py -q
1 passed in 183.45s (0:03:03)
```

> 注意 `1 failed in 0.38s`：缺数据时**不做任何入库**，所以验收红得非常快 —— 这正是想要的
> 失败模式（不是跑了三分钟才告诉你没数据）。而 `raise_if_any()` 是**收集式**的：数据缺失时
> 契约侧四条仍会被执行完（输出里可见 `17 fixtures, 0 failures`），不会因为第一条红就漏报。

**(d) 变异证明（2 条，均为 pytest 原生：跑的是 `test_m1_acceptance` 本身，不是复刻的 SQL）**。
M1 的每条断言都必须能被真实的状态变化打红，否则「全绿」只是「没测」。

**变异 1 · 删掉 `heroes` 里的一行（hero_id=121）**

真实英雄**全部**被引用（该英雄 51,233 手 + 1 行 token 索引），提交式的 `DELETE` 会被 FK 拒
（实测 `ForeignKeyViolation: draft_actions_hero_id_fkey`）—— 故变异在**一个未提交事务**里丢掉
两条 FK 后施加，跑完回滚。打红 3 条，且每条都带「提升 `SNAPSHOT_VERSION`」的可操作指引：

```
变异已施加（未提交）：heroes 少了 hero_id=121；当前 heroes = 126 hero_token_index = 126
M1 验收未通过（3 条）：
✗ M1-1 常量表英雄数（§14 M1）：实测 126，要求 127 —— 若这是**上游例行变化**（新英雄 / 新版本 /
  新数据集），按规格 §5.1 提升 constants.load.SNAPSHOT_VERSION 并重派生 dense_index（= 重训模型）；…
✗ M1-6 hero_token_index 行数（§5.1/§10.1）：实测 126，要求 127 —— …
✗ M1-6 dense_index 派生规则（§5.1:214–216）：实测 11，要求 0 —— dense_index =
  row_number() OVER (ORDER BY hero_id) - 1；不一致说明映射被改过。
===== 回滚后复原校验 =====
heroes = 127 hero_token_index = 127
```

第 3 条是**反向**打红（少一行会让其后整体位移，11 个 hero_id 的映射错位）—— 它正是
§5.1:214–216 论证里「新英雄插进空位导致其后整体位移」的可观测形态。

**变异 2 · 把 4,000 场的 `matches.anomaly` 置 true（不动 `draft_anomalies`）**

同一事务内跑三次，构成完整闭环（基线 → 变异 → 复原）：

```
===== ① 基线（无变异，期望 PASS） =====      → PASSED
变异已施加（未提交）：4000 场 anomaly 置 true，draft_anomalies 不动
===== ② 变异（+4000 场 anomaly，期望 FAIL） =====
→ FAILED：M1 验收未通过（7 条）：
✗ M1-4 异常率 < 2%（§14 M1 + §15①，全库分母）：实测 '6048/211051 = 2.87%'，要求 < 2% —— …
✗ M1-4 异常场次绝对数 = 实测值 ±5%（§15①）：实测 6048，要求 2048 ± 5% —— …
✗ M1-4 draft_anomalies 与 matches.anomaly 互证（§5.3）：实测 '2048 vs 6048'，要求 相等 —— …
✗ M1-10 族分类分母（§15③）：实测 199022，要求 202957
✗ M1-10 入库的 anomaly 与独立 SQL 判定互证（§5.3）：实测 3935，要求 0
✗ M1-10 逐族支持度 = 实测分布（§15③）：cm20_a 10,572（应 12,340）等
✗ M1-10 十族之和 = 已分类总数（§15③）：实测 199022，要求 202957
===== ③ 回滚复原后（期望 PASS） =====      → PASSED
```

**变异 2b · 只改 500 场（比率仍 < 2%）** —— 专门证明「只钉比率」会漏掉的东西确实被盯住了：
`anomaly` 从 2,048 掉到 548，比率 0.26% **仍然合格**，但绝对数越出 ±5% 带，被
`M1-4 异常场次绝对数` 抓住（17 条复算里只有这一条变红）。换句话说：`< 2%` 单独存在时，
**loader 少判 4,000 场异常**这种事是抓不到的；那条 ±5% 的带子就是为它加的。

**复原与清理**：变异用的一次性 scratch 库（`dota_mut`）跑完即 DROP；正式验收链
（`tests/test_m1_acceptance.py` → 根 conftest 的 `db_after_bootstrap`）从不被变异触碰 ——
它每次 session 都 `DROP + CREATE <db>_test_bootstrap`，跑完在 teardown 里再 DROP
（实测跑完全量套件后 `pg_database` 里**没有** `*_bootstrap` 残留，共享 `dota_test` 的 11 张
数据表全为 0 行）。
