...
更新：2026-09-22。当前交付对局实验室与完整实验计算链，GUI 与独占 PostgreSQL 保持运行。最新记录以 `docs/reviews/2026-09-22-analysis-lab.html` 和同名 JSON 为准。最新验收与完整能力核查见 `docs/reviews/2026-09-22-workbench-redesign.html` 与同名 JSON；上一版验收见 `2026-09-22-workbench.html`；位置来源快照仍以 `2026-09-22-m3-provenance.json` 为准。
...
| M4/M5 | Value 与七体系 Playbook 实验 API 已接入；预测质量未独立验收，条件化对位真实样本仍不足 |
| M6 | 本地 GUI 已接入双方对局、24手编辑、真实前缀、四模块输出和JSON；保留全部画像功能 |
| M7/M8 | 条件概率 Policy 已训练并在线可用，标记 experimental；神经序列模型及新的独立质量门未完成 |
| M9/M10 | Advise 实验搜索已接入，全对手候选计算期望和最差值；正式发布和部署未完成 |
...
此前只有 Profile。此次新增 Value、Policy、Playbook、Advise 的真实服务与模型，入口为 `/#analysis`。双方历史胜负、平均时长和直接交手已经接入；天梯采集、回放与位置真值仍未完成。当前仅可验收完整实验路径，不能称预测质量已正式通过。
...
3. 收集新的未读时间尾段做独立质量验收；现有 Value/Policy 均为开发复用验证。继续更强序列模型、历史 CM 池和条件化对位样本，禁止通过调低样本门槛使页面变绿。
...
本轮根基线为 `a17efc2`。三路实施分别拥有 Value、Policy、决策引擎，主控拥有公开请求、服务编排、HTTP 与 GUI。全部模型训练已结束，无后台采集或悬空训练。

- `analysis/analysis_service.py` 编排四模块；响应包含 `decision_mode=experimental`、`release_ready=false`。推演消费已训练的实验模型，不表示正式质量门通过。Value 未优于固定胜率基准时不进入 Advise/Playbook。
- `prepare_value` 和 `prepare_policy` 单次只读准备；搜索复用纯函数并按请求缓存，避免每个候选重新连库。
- 当前 e 模型和 Policy 开发评估本地可用，默认空局面真实 HTTP 为 8.324 秒，8候选、3方案、7分支。7.41f Value 模型不足，Policy仍可显示，依赖模块逐项说明不足。
- 样例 8996973546 的前12手已在真实浏览器跑通四模块；选择当前行动方为我方，保留真实天辉夜魇身份。样例可能被当前模型看过，因此不是无泄漏历史回测。
- 493项全仓 Python 测试通过，耗时179.26秒；11项前端测试通过。日志 `data/analysis-final-pytest.log`；HTTP证据 `data/reviews/analysis-http-acceptance.json`。新增真实渲染故障已修并补测试。
- 独立审查修复了 UTC/上海时区边界、截止后赛果泄漏、pub来源遗漏、系列赛换边、单模块失败隔离、候选不足与positions显式降级。
- 模型文件与原始数据不提交。模型 SHA-256 和聚合指标写入审阅 JSON。复算命令见 README。

当前 Policy 为条件概率模型，不是规格远期计划的 LSTM 或 Transformer。开发验证7248预测，Top1 9.95%、Top5 30.13%；阶段基准8.71%、24.63%。Value e 准确率50.58%，固定胜率基准51.74%；log loss有很弱提升。不要只摘取较好指标声称成熟。d 的log loss劣于基准，禁止进入决策搜索。历史CM池仍缺权威快照，选手counter真实样本不足，位置仍为历史规则推断。
