...
更新：2026-09-22。当前已推进到 M3 画像后端和真实 HTTP 验证。旧采集快照保留，当前数据量与测试结果以本轮 m3 报告为准。

## 工作区与入口

工作目录：`/Users/xinzechao/MCNDOTAGA/.worktrees/plan-1-contract-and-data-foundation`。
分支：`plan-1-contract-and-data-foundation`，基线 `2c244a7`。此处记录 M3 验收快照；首次 GitHub 检查点包含职业采集、画像后端、335 项测试和复算脚本。没有部署。
...
4. `docs/reviews/2026-09-22-m3-profile.html`、`2026-09-22-m3-validation.json`。
...
| M3 画像 | 公开画像计算、查询、只读 HTTP、错误契约和静态报告已实现；真实英雄池、原型、BP 可用，真实五个位置维度仍因位置未知降级 |
...
已有选手详情 1,562 场，上轮为 21 场。位置 NULL 的参赛记录 15,620 条，不从 slot、英雄或当前 roster 猜位置。
...
- premium/professional 不是 tier1/tier2 的同义词。repository 保留未知，不能擅自映射；未知目标层级的同侪只允许 tier1/tier2。
...
完整测试：335 passed，314.10 秒，比 M2 的 260 项新增 75 项。使用独立 `mcndotaga_m3_final_test`，日志 `data/m3-final-pytest.log`。
实际启动 CLI HTTP 服务，在开发库请求 4 支真实队伍，4 支成功返回通过冻结 schema 和不变式的画像。拒绝训练赛、未知队伍、超早日期三条实际 HTTP 错误路径通过。临时服务已退出。
...
自动浏览器视觉未验证。此前工具拒绝本地 file URL，没有换浏览器或代理绕过。静态 HTML 的数据、转义、结构与禁用排版做机械检查；这些不等于视觉验收。48 小时稳定性、真实位置百分位与生产部署也未验收。
...
TEST_DATABASE_URL=postgresql://dota@127.0.0.1:55439/mcndotaga_m3_final_test .venv/bin/python -m pytest -q
...
2. 用现有 lane_role、GPM、XPM、补刀、净经济实现可复算的位置推断，并保留来源、方法版本与置信度，不能冒充提供方位置。按 league_id 补有证据的赛事层级映射，再用真实样本验收五维百分位。数据缺口审计见 `docs/reviews/2026-09-22-m3-data-gaps.json`。
