...
## ✅ 已裁决（选项 1，2026-09-18）：24 手模板是「2026 年族」的定义

**这是 Task 12 最重要的发现，不是 bug，是规格假设被真实数据推翻；现已由人拍板按选项 1 落地。**

Task 12 用真实 Kaggle 全库（**211,051 场**）实测：

| 事实 | 数值 |
|---|---|
| `shared/draft_template.TEMPLATE`（规格 §6.0/§8①，Task 4 的"唯一定义处"）对**有 BP 序列的 2026 场次**的匹配率 | **99.59%**（14,131/14,189；其余 58 场是异常）—— 上一版写"100%"是没扣掉异常场 |
| 同一模板对全库（2016–2026）的匹配率 | **7.17%**（15,135/211,051）；占**有 BP 序列**场次 7.38%、占 **24 手**场次 **10.12%**（上一版只写 10.12%，容易被读成全库） |
| 全库真实存在的**合法手序族** | **10 种**（唯一权威已移到 `ingest/order_families.DRAFT_ORDERS`；`load_bootstrap` 只 re-export） |
| 按计划字面规则（"手数≠24 即异常"）算出的异常率 | **58.33%** —— 直接违反 M1 的 `< 2%` |
| 实际采用的异常口径（"不匹配任何实测合法手序族"） | **0.97%** ✅ |

**为什么这需要人拍板**：`resolve()` 的 `(is_pick, team)` 推导是契约的一部分（Task 6 的 `check_playbook` 用它校验 `by_ord` 归属、Task 8 的 TS 契约、线 C 的位置 token 都由它派生）。若历史比赛的手序族不同，则：
...
**裁定（2026-09-18，选项 1）**：**契约保持冻结**（示例与前端仍以 24 手模板为准），**族感知下沉到
ingest/analysis/models**。规格已按此澄清（§6.0 新增一段、§5.3/§8①/§10.1/§15 口径同步）；代码侧
`ingest/order_families.py` 是可复用的唯一入口，`tests/ingest/test_order_families.py` 全量守护
（202,957 场零未分类、恰好一族）。**任何按 `resolve()` 做归属的下游代码必须先确认场次所属族** —— 这
一条没有变。
